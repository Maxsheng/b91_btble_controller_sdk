 ...
 */
_attribute_ram_code_sec_  unsigned char uart_send_dma(uart_num_e uart_num, unsigned char * addr, unsigned int len )
{
	if(len!=0)
	{
 ...
 * @return    none
 */
_attribute_ram_code_sec_noinline_ uart_set_rx_dma_enable(uart_num_e uart_num, u8 en)
{
	if(!en){
		dma_chn_dis(uart_dma_rx_chn[uart_num]);
	}
