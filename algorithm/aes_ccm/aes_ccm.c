...
void aes_ll_encryption(u8* key, u8* plaintext, u8 *encrypted_data)
{
    ...
    reg_aes_ptr = (u32)((u32)aes_data_buff - EM_BASE_IRAM_OFFSET);
 ...
 * @Note		Input data requires strict Word alignment
 */
/* for Eagle, call 3 times in ram_code, static_inline change to ram_code function, save 126 Byte RAM */
#if (MCU_CORE_TYPE == MCU_CORE_9518)
_attribute_ram_code_
#else
static inline
#endif
void aes_feed_data(u8 plaintext[16])
{
	u8 *ptr = plaintext + 12;
    for (int i=0; i<4; i++){
 ...
 * @Note		Input data requires strict Word alignment
 */
/* for Eagle, call 3 times in ram_code, static_inline change to ram_code function, save 36 Byte RAM */
#if (MCU_CORE_TYPE == MCU_CORE_9518)
_attribute_ram_code_
#else
static inline
#endif
void aes_read_result(u8 result[16])
{
	u8 *ptr = (u8 *)&aes_data_buff[4];
	for (int i=0; i<16; i++) {
		result[i] = ptr[15 - i];
...
int aes_ll_ccm_decryption_v2(leCryptCtrl_t *pd)
{
	...
	*pPduLen -= 4; //minus 4 byte mic length
    ...
    return r;
		...
		if(smemcmp(ARaw, BRaw, sizoeof(BRaw))){
			//printf("TEST ERR\n");
		}
	#endif
#endif
