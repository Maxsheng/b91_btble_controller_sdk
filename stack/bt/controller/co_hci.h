...
/// hci role discovery command parameters structure
struct hci_role_disc_cmd
{
    ///Connection handle
    uint16_t conhdl;
};
...
struct hci_link_key_reply_neg_cmd
{
    ///BdAddr
    struct bd_addr  bd_addr;
};
...
struct hci_pin_code_reply_neg_cmd
{
    ///BdAddr
    struct bd_addr  bd_addr;
};
...
/// hci exit sniff mode request command parameters structure
struct hci_sniff_mode_exit_cmd
{
    ///Connection handle
    uint16_t    conhdl;
};
...
typedef struct hci_create_con_cmd
{
    ...
    ///Allow Switch
    uint8_t         switch_en;
}__attribute__ ((__packed__))  hci_create_con_cmd_t;

typedef struct hci_create_conn_cancel_cmd
{
    ///BdAddr
    struct bd_addr  bd_addr;
}__attribute__ ((__packed__))  hci_create_conn_cancel_cmd_t;
...
///HCI remote name request cancel command structure
struct hci_rem_name_req_can_cmd
{
    ///BdAddr
     struct bd_addr  bd_addr;
};
...
typedef struct hci_setup_sync_con_cmd
{
    ...
    ///Packet type
    uint16_t pkt_type;
}__attribute__ ((__packed__)) hci_setup_sync_con_cmd_t;
...
typedef struct hci_enh_setup_sync_con_cmd
{
    ...
    uint16_t      packet_type;          // Packet Type
    uint8_t       retx_eff;             // Retransmission Effort
}__attribute__ ((__packed__)) hci_enh_setup_sync_con_cmd_t;
...
///HCI user confirm request reply command structure
struct hci_user_cfm_req_reply_cmd
{
    ///BdAddr
    struct bd_addr  bd_addr;
};
...
///HCI user confirm request reply command structure
struct hci_user_cfm_req_reply_neg_cmd
{
    ///BdAddr
    struct bd_addr  bd_addr;
};
...
/// HCI Accept synchronous connection request command structure
struct hci_set_esco_mute_cmd
{
    uint16_t    esco_handle;

    uint8_t     mic_spk; //bit0, mic; bit1 spk

    uint8_t     mute_en;
};

/// HCI Write Link Supervision Timeout command structure
struct hci_write_link_supervision_timeout
{
    /// Connection handle
    uint16_t conhdl;
...
struct hci_rd_ext_page_to_cmd_cmp_evt
{
     ...
     * Range for N: 0x0000 (default) 锟�0xFFFF
...
}__attribute__ ((__packed__)) hci_inq_res_evt_t;
...
}__attribute__ ((__packed__)) hci_inq_res_with_rssi_evt_t;
...
typedef struct hci_ext_inq_res_evt
{
    ...
    struct eir      eir;
}__attribute__ ((__packed__)) hci_ext_inq_res_evt_t;
...
#define MAX_HOST_IND_LEN 20
#define MAX_HOST_CMD_LEN 25

struct dbg_ind2host_cmd
{
	uint8_t ind2host_buf[MAX_HOST_IND_LEN];
};

struct dbg_tl_itf_mode_cmd
{
	uint8_t tl_itf_mode;
};

struct dbg_fixed_freq_cmd
{
	uint8_t tx_freq;
	uint8_t rx_freq;
};

struct dbg_rf_tone_cmd
{
	uint8_t tx_freq;
};

///HCI Debug bt bd address write command parameters -vendor specific
struct dbg_set_bt_bd_addr_cmd
{
	///bd address to set
	struct bd_addr addr;
};
