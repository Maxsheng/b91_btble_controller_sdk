...
uint32_t bt_ll_system_tick_bt_acl_isr(uint8_t sequence, int link);
uint32_t bt_ll_system_tick_access_isr(uint8_t sequence, int link);
...
#if PROJ_CUSTOM_AG// temp add for test
	SCHEDULER_PRIORITY_BLE			=	0x20,
...
typedef	uint32_t (*func_system_tick_isr_t) (uint8_t sequence, int link);
...
{return (env_sch.mode & SCHEDULER_MODE_TWS) && (!(FLAG_SCHEDULE_LINK_ID_NULL & env_sch.next_id) || (FLAG_SCHEDULE_LINK_ID_NULL & env_sch.cur_id));}
